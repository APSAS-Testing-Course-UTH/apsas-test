...
describe('PUT /api/v1/users/{userId}/deactivate - Deactivate User', () => {
  ...
  it('should deactivate user successfully for admin', async () => {
    const testUserId = 'student-001' // Use a student user for deactivation

    const response = await fetch(`/api/v1/users/${testUserId}/deactivate`, {
      method: 'PUT',
      headers: {
    ...
    expect(data).toHaveProperty('isActive', false) // Should be deactivated
  ...
  it('should return 404 for non-existent user', async () => {
    ...
    const response = await fetch(`/api/v1/users/${nonExistentId}/deactivate`, {
      method: 'PUT',
      headers: {
        'Authorization': `Bearer ${mockTokens.admin}`,
        'Content-Type': 'application/json',
  ...
  it('should handle special characters in user ID', async () => {
    ...
    const response = await fetch(`/api/v1/users/${encodeURIComponent(specialId)}/deactivate`, {
      method: 'PUT',
      headers: {
        'Authorization': `Bearer ${mockTokens.admin}`,
        'Content-Type': 'application/json',
  ...
  it('should handle instructor trying to deactivate user', async () => {
    const testUserId = 'student-001'
