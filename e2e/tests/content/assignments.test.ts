...
async function selectMultiSelectValue(
  I: CodeceptJS.I,
  placeholder: string,
  value: string,
) {
...
async function grabDraftTitlesOnCurrentPage(
  I: CodeceptJS.I,
...
async function grabVisibleAssignmentTitlesOnCurrentPage(
  I: CodeceptJS.I,
  expectedCount: number,
): Promise<string[]> {
  const titles = await I.executeScript((count: number) => {
...
async function assertAssignmentStatusByTitle(
  ...
  if (info.label !== expectedLabel) {
    throw new Error(
      `Trạng thái assignment không đúng. expected=${expectedLabel}, actual=${info.label}, title=${title}`,
    );
  }
...
async function publishAssignmentPreferDetail(
  I: CodeceptJS.I,
  assignmentId: string,
  ...
  const publishButtonsInDetail = await I.grabNumberOfVisibleElements(
    locate("button").withText("Công bố"),
  );
...
async function isStudentAssignmentVisibleInAnyPage(
  I: CodeceptJS.I,
  title: string,
): Promise<boolean> {
...
Scenario(
  "[C-01][C-02][C-03][C-05][C-06] provider tạo, cập nhật, gán tài nguyên và xuất bản bài tập",
  async ({ I }) => {
    ...
    allure.story(
      "content provider creates, updates, enriches, and publishes an assignment",
    );
    ...
    await I.amOnPage("/provider/assignments");
    await I.waitForText("Quản lý bài tập", 10);
    await assertAssignmentStatusByTitle(
      I,
      persistedTitleAfterAdvanced || updatedTitle,
      "Đã công bố",
      "blue",
    );
...
Scenario("[C-04] student chỉ nhìn thấy bài tập đã xuất bản", async ({ I }) => {
  ...
  allure.story(
    "student only sees published assignments in my assignments list",
  );
  allure.severity("critical");
  allure.tag("e2e");
  allure.tag("regression");
...
Scenario(
  "[C-04-Extended] student cannot access draft assignment directly",
  async ({ I }) => {
