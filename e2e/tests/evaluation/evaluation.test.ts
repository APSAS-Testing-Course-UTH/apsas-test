...
async function loginAsStudent(I: CodeceptJS.I, email: string): Promise<void> {
  ...
  throw lastError instanceof Error ? lastError : new Error(`Failed to login as student ${email}`)
}

Feature("Evaluation Service")

Scenario("EVL-E2E-001: Show evaluated summary for seeded submission", async ({ I }) => {
...
Scenario("EVL-E2E-002: Display PASSED result for correct code", async ({ I }) => {
  await allure.epic("evaluation")
...
Scenario("EVL-E2E-003: Display FAILED result for wrong logic", async ({ I }) => {
  await allure.epic("evaluation")
  await allure.feature("evaluation flow")
...
Scenario("EVL-E2E-005: Instructor feedback is visible to student", async ({ I }) => {
  await allure.epic("evaluation")
